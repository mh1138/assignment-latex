...
\item Cloud Computing wird in einer Diplomarbeit erklärt.\footcite[S.~14]{Boettger:Diplomarbeit} (siehe dazu Fußnote~\ref{fn:zitierfaehig} auf Seite~\pageref{fn:zitierfaehig})
...
\item Die DHBW hat neun Standorte, drei Campus sowie das CAS.\footcite[Abschnitt~\enquote{DHBW Standorte}]{onl:DHBW}
...
\item zwei Beiträge zu Büchern\footnote{\cite{Trautwein:Nokia};\cite{Mann}} und zu einem Konferenzband\footcite{Trautwein:Erfolgsfaktoren}
...
\begin{itemize}
\item \emph{Zwei Quellen am Satzende} werden in einer gemeinsamen Fußnote mit Semikolon getrennt zusammengefasst.\footnote{
Vgl. \cite[S. 20]{Staab}; \cite[S. 1234--2920]{mayerLukas:PA1}}
\item \emph{Eindeutigkeit:} Normalerweise wird kein Vorname des Autors im Zitat angegeben. Falls es allerdings zur Eindeutigkeit\footnote{\cite{trautwein2011unternehmensplanspiele} vs. \cite{hitzler2011optimierung}} (bei gleicher Jahreszahl) erforderlich ist, wird der Vorname abgekürzt bzw.\ nötigenfalls sogar ganz ausgeschrieben mit angegeben.\footnote{Vgl. \cite{mayer:PA1} und \cite{mayerLukas:PA1}}
 
Welch ein Glück, dass Sie sich darum dank \LaTeX\ gar nicht kümmern müssen (arme Word\texttrademark-User ;-).

\item \emph{middle initial}: Im Literaturverzeichnis soll bei den Autorinnen und Autoren der erste Vorname ausgeschrieben, ein möglicher zweiter Vorname allerdings nur abgekürzt werden (letzteres ist v.a.\ in Nordamerika gebräuchlich). Bei Personen, die mehr als einen Vornamen haben, ist bereits in der Literaturdatenbank (wie z.B.\ beim Eintrag \cite{Ancuti}) der zweite Vorname abzukürzen, da dies aktuell nicht von \LaTeX\ automatisch übernommen werden kann. 
...
\item Zwei Quellen vom selben Verfasser, aber ohne Jahresangaben.\footnote{
\cite{onl:DHBWLeitung}; \cite{onl:DHBW}}
Handelt es sich um Online-Quellen, so ist Fußnote~\ref{fn:onlineEntryType} zu beachten.
...
\item[thesis] Bachelor-/Master-/Diplomarbeit\footnote{\label{fn:zitierfaehig}Beachten Sie bitte, dass solche Arbeiten üblicherweise nicht als zitierfähige Quellen gelten.} (Art wird im Attribut \enquote{type} festgelegt)
\item[online] Internet- oder Intranet-Quelle\footnote{\label{fn:onlineEntryType}Man beachte, dass der Eintragstyp \enquote{online} in JabRef nur im \enquote{biblatex-Modus} (Menü: Datei -- Neue biblatex Bibliothek) auswählbar ist. Sofern kein Jahr oder Datum bekannt ist, sollte im Attribut \emph{year} \enquote{o.J.} bzw.\ im Englischen \enquote{w.y.} eingetragen werden, sonst 
fügt \LaTeX\ die aktuelle Jahreszahl ein.}
...
Eine Übersicht über die notwendigen Attribute jedes Eintragstyps gibt Tabelle~\ref{tab:eintragstypen}, wobei ein Schrägstrich als \enquote{oder} zu verstehen ist.\footnote{Auszugsweise entnommen aus \cite{biblatex:manual}.} Zudem sind die wichtigsten optionalen Attribute aufgeführt.
...
\hline
online\textsuperscript{\ref{fn:onlineEntryType}} & author/editor, title, year/date, url & urldate \\
\hline
report & author, title, institution, type, year/date & number, version, url, urldate \\
\end{tabular}
\caption{Attribute der Eintragstypen.}
\label{tab:eintragstypen}
\end{table}
...
Ins Literaturverzeichnis soll nur die Sekundärquelle aufgenommen werden. Dies wird dadurch erreicht, dass in der Literatur-Datenbank bei der Primärquelle im Attribut \enquote{keywords} der Wert \enquote{ausblenden} eintragen wird.

Im Folgenden sehen Sie einen Auszug aus der Literatur-Datenbank mit einer Primär- und einer Sekundärquelle. 

\lstinputlisting[firstline=236, breaklines, numbers=none]{includes/literatur-datenbank.bib}