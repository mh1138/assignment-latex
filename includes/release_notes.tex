...
\anhangteil{Änderungen in Version 2.0}\label{anhang:ReleaseNotes20}
Einer studentischen Anregung folgend wurde in diese Version eine Konfigurationsdatei eingeführt, in der sich übersichtlich alle nötigen Einstellungen machen lassen. Außerdem wird nun dort ein einfaches Umschalten zwischen deutsch- und englischsprachigen Arbeiten ermöglicht, so dass keine weiteren händischen Anpassungen erforderlich sind. Die Vorlage enthält nun auch eine Erklärung zum Einsatz von KI-Werkzeugen.\footnote{%
Danke an Fabian Fritz, Jonas Michel, Lennart Schnee und Max Weiberle sowie Prof. Dr. Offtermatt für ihre Hinweise und Unterstützung bei diesem Update.}

\begin{itemize}
\item \verb|latex_vorlage.tex| (Hauptdokument)
\begin{itemize}
\item Zu Beginn wird die Konfigurationsdatei \verb|config.tex| eingebunden.
\item Die Option \verb|listof=no chaptergap| im Befehl \verb| \documentclass| behebt das Problem, dass im Abbildungs- und Tabellenverzeichnis ein vertikaler Abstand zwischen Einträgen unterschiedlicher Kapitel war.
\item Es wird die Datei \verb|erklaerung_ki.tex| eingebunden.
\end{itemize}
...
\item \verb|abkuerzungen.tex|
\begin{itemize}
\item Das Abkürzungsverzeichnis wird nun mit dem Befehl \verb|\startAbkVerzeichnis| eingeleitet.
\end{itemize}

\item \verb|anhang.tex|
\begin{itemize}
\item Der Anhang wird nun mit dem Befehl \verb|\startAnhang| eingeleitet.
\end{itemize}

\item \verb|config.tex|
\begin{itemize}
\item Datei wurde neu hinzugefügt.
\item Hier werden nun zentral alle Einstellungen (Typ, Titel, Autor/in der Arbeit etc.) vorgenommen, die bisher in \verb|deckblatt.tex| verstreut waren.
\item Ein Umschalten zwischen Deutsch und Englisch ist hier möglich.
\end{itemize}

\item \verb|deckblatt.tex|
\begin{itemize}
\item Datei wurde entfernt. 
\item Bisherige Inhalte finden sich nun in \verb|config.tex| und \verb|_dhbw_deckblatt.tex|. 
\end{itemize}

\item \verb|_dhbw_authoryear.bbx|
\begin{itemize}
\item Die Nummerierung mehrerer Quellen ohne Jahreszahl desselben Autors wurde den Zitierrichtlinien angepasst.
\end{itemize}

\item \verb|_dhbw_biblatex-config.tex|
\begin{itemize}
\item Anpassungen für englischsprachige Arbeiten.
\end{itemize}

\item \verb|_dhbw_deckblatt.tex|
\begin{itemize}
\item Datei enthält das Deckblatt gemäß der DHBW-Vorgaben.
\item Es sind hier keine Änderungen erforderlich, alle Einstellungen werden in \verb|config.tex| vorgenommen.
\item Englischsprachige Arbeiten werden unterstützt.
\end{itemize}
...
\begin{itemize}
\item Englischsprachige Arbeiten werden unterstützt.
\item Zur Vereinfachung neue Befehle \verb|\startAnhang| und \verb|\startAbkVerzeichnis| definiert.
\end{itemize}


\item \verb|einleitung.tex|
\begin{itemize}
\item Hinweise auf Editoren ergänzt.
\item Erläuterung der Konfigurationsdatei.
\end{itemize}

\item \verb|erklaerung_ki.tex|
\begin{itemize}
\item Datei wurde neu hinzugefügt.
\item Hier ist anzugeben, welche KI-Tools verwendet wurden.
\end{itemize}

\item \verb|literatur-datenbank.bib|
\begin{itemize}
\item Bei einzelnen Einträgen war im Attribut \enquote{year} der Wert \enquote{o.J.} angeben, was bei einer englischsprachigen Arbeit falsch ausgegeben wird.
\item Überflüssige und/oder vom Literaturverwaltungsprogramm generierte Attribute wurden entfernt.
\end{itemize}

\end{itemize}

\anhangteil{Änderungen in Version 2.1}\label{anhang:ReleaseNotes21}

In dieser Version wurde der Wortlaut der Erklärung zum Einsatz von KI-Werkzeugen angepasst, um die zwischenzeitlichen Änderungen abzubilden. Außerdem wurden einige Fehler korrigiert.

\begin{itemize}

\item \verb|latex_vorlage.tex| (Hauptdokument)
\begin{itemize}
\item In der Vorversion wurde die Option \verb|listof=nochaptergap| im Befehl \verb| \documentclass| aufgenommen, allerdings fälschlicherweise mit einem überflüssigen Leerzeichen. Dieser Fehler ist jetzt korrigiert.
\end{itemize}

\item \verb|abbildungen_und_tabellen.tex|
\begin{itemize}
\item Fußnoten in Beschriftungen der Abbildungen korrigiert.
\item Erklärungen zur professionellen Gestaltung von Tabellen ergänzt.
\item Beispiel für Einsatz des Pakets amsmath.
\end{itemize}

\item \verb|abkuerzungen.tex|
\begin{itemize}
\item Hinweis auf Paket-Option ergänzt.
\end{itemize}
...
\item \verb|_dhbw_biblatex-config.tex|
\begin{itemize}
\item Vornamen der Autor/innen werden nun ausgeschrieben.
\item Namen der Autor/innen werden nun mit Komma (in der Fußnote) bzw.\ Semikolon (Literaturverzeichnis) anstelle eines Schrägstrichs getrennt.
\item Im Kurzbeleg soll im Deutschen \enquote{et al.} und nicht \enquote{u.a.} verwendet werden.
\end{itemize}

\item \verb|_dhbw_praeambel.tex|
\begin{itemize}
\item Es wird das Paket amsmath eingebunden für erweiterten Formelsatz.
\item Es wird das Paket amssymb eingebunden, um die Symbole $\square$ und $\boxtimes$ darstellen zu können.
\item Beim Paket acronym wird die Option \verb|nohyperlinks| gewählt.
\item Für biblatex wird die Option \verb|dashed=false| gesetzt, um bei mehreren Quellen einer Autorin deren Namen im Literaturverzeichnis auszuschreiben (anstelle eines Gedankenstrichs).
\item Die Option \verb|hypertexnames=false| behebt eine Warning für das Paket hyperref.
\end{itemize}


\item \verb|erklaerung_ki.tex|
\begin{itemize}
\item Textvorlage wurde angepasst, da die Erklärung auch abzugeben ist, wenn keine KI-Systeme eingesetzt wurden.
\end{itemize}

\item \verb|literatur-datenbank.bib|
\begin{itemize}
\item Vornamen der Autorinnen und Autoren sind jetzt ausgeschrieben.
\item Ein Eintrag mit fehlerhaften Attributen wurde korrigiert. 
\end{itemize}

\item \verb|text_mit_zitaten.tex|
\begin{itemize}
\item Mehrere Quellen an einer Stelle werden durch Semikolon getrennt in der Fußnote dargestellt.
\item Hinweis auf Zitierfähigkeit.
\item Handhabung \enquote{middle initial} erklärt.
\item In der Anleitung zur Verwendung von Sekundärliteratur (Abschnitt~\ref{sec:sekundaerliteratur}) muss das Attribut \enquote{keyword\textbf{s}} heißen. Dies wurde korrigiert und durch ein Beispiel illustriert.
\end{itemize}
