...
Sämtliche benötigten Werkzeuge sind Open Source und damit kostenlos nutzbar. Sie benötigen i.W.\ einen Compiler, einen Editor und ein Programm zur Verwaltung der Literatur-Datenbank. Falls Sie bereits mit \LaTeX\ gearbeitet haben, können Sie diesen Abschnitt überspringen.

\subsection{Compiler}
Für einen einfachen Start in \LaTeX\ sowie Tests können Sie einen \emph{Online-Editor} wie \url{overleaf.com} verwenden. Allerdings sollten Sie eine Projekt- oder Bachelorarbeit nicht mit diesem System erstellen, da es sich um eine Cloud-Lösung handelt, bei der letztlich nicht gewährleistet ist, dass Firmen-Interna und schützenswerte Daten nicht in die Hände Dritter gelangen können. 

Für eine \emph{lokale Installation} können Sie z.B.\ TeXLive (alle gängigen Plattformen, \url{http://tug.org/texlive/}), MikTeX (Windows, \url{http://www.miktex.org/}) oder TexShop (Mac OS, \url{http://pages.uoregon.edu/koch/texshop/}) verwenden.

In Moodle findet sich der Link zu einer \emph{portablen Version} von MikTeX, welche ohne Installation auskommt. Dieses System wird für Schulungen an der DHBW genutzt, es enthält aber prinzipiell alle Werkzeuge, um eine Bachelorarbeit zu schreiben. Mit diesem System wird auch jede neue Version der Vorlage getestet.

\subsection{Editor}
Im Grunde eignet sich jeder Texteditor oder jede Entwicklungumgebung (IDE), um \LaTeX-Source Code zu schreiben. In Wikipedia\footnote{\url{https://de.wikipedia.org/wiki/LaTeX}} finden Sie eine Liste möglicher Editoren. In der oben erwähnten portablen Version ist der relativ schlichte, aber einfach zu bedienende, Editor TeXworks enthalten. Sie können mit diesem starten und jederzeit auf einen anderen umsteigen.
...
um den vorgegebenen Stil der Zitierrichtlinien umzusetzen. Der Editor muss daher so konfiguriert werden, dass er beim Übersetzen \emph{biber} statt \emph{bibtex} verwendet. 

Zur Verwaltung der Quellen in einer eigenen Datei nutzt man etwa JabRef (Java, \url{http://jabref.sourceforge.net/}) oder BibDesk (Mac OS, \url{https://bibdesk.sourceforge.io}).


\section{Verwendung der Vorlage}

Editieren Sie bitte an folgenden Stellen:
\begin{itemize}
\item in der zentralen Konfigurationsdatei \verb|config.tex|,
\item in den unten angegebenen Abschnitten des Hauptdokuments, um Ihren Inhalt einzubinden sowie
\item in der Erklärung über die Nutzung von KI-Werkzeugen in der Datei \verb|erklaerung_ki.tex|, welche unmittelbar vor der ehrenwörtlichen Erklärung stehen soll.
\end{itemize}


\subsection{Zentrale Konfigurationsdatei}
\label{sec:konfigurationsdatei}

Alle Einstellungen nehmen Sie in der Datei \verb|config.tex| an den mit \enquote{TODO} gekennzeichneten Stellen vor. Diese Datei  wird ganz zu Beginn des Hauptdokuments eingebunden und sollte selbsterklärend sein. 

Folgende Einstellungen sind \emph{verpflichtend} vorzunehmen:
\begin{itemize}
\item Art Ihrer Arbeit (Projekt-, Bachelor- oder Seminararbeit)
\item Ihr Name
\item Ihr Kurs
\item Titel Ihrer Arbeit (dieser wird auch automatisch in die ehrenwörtliche Erklärung übernommen)
\item Angaben zum Unternehmen (wie sie auf dem Deckblatt erscheinen)
\item DHBW-Betreuer/in
\end{itemize}

\emph{Optionale} Einstellungen betreffen:
\begin{itemize}
\item die Sprache Ihrer Arbeit (Standard ist Deutsch)
\item Schriftgröße und Umbruch des Titels auf dem Deckblatt (falls dieser besonders lang ist)
\item das Abgabedatum
\item einen möglichen Vertraulichkeitsvermerk\footnote{%
...
Eine Umstellung der Sprache auf \enquote{EN} (Englisch) wirkt sich auf Deckblatt und Bezeichnungen (Verzeichnisse etc.) aus.

\subsection{Hauptdokument}

Sie bearbeiten folgenden Abschnitt, um Ihre einzelnen Kapitel einzubinden:
\lstset{language=}
\lstinputlisting[firstline=51,lastline=61,firstnumber=51]{latex-vorlage.tex}
...
\lstinputlisting[firstline=24,lastline=45,firstnumber=24]{latex-vorlage.tex}
...
_dhbw_deckblatt.tex        (Deckblatt der Arbeit)
...
\subsection{Falsch dargestellte Umlaute}
...
\subsection{Farbig hervorgehobene Links}

Die farbigen, anklickbaren Links, die in der PDF-Ansicht enthalten sind, werden beim Ausdruck nicht wiedergegeben, erleichtern aber das Lesen der elektronischen Fassung.

Das Feature lässt sich abschalten, indem man die Option \texttt{hidelinks} bei \texttt{documentclass} zu Beginn des Hauptdokuments hinzufügt. Für weitere Konfigurationsmöglichkeiten siehe \url{https://ctan.org/pkg/hyperref}.
